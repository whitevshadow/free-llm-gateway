...
const COMBO_TEMPLATE_FALLBACK = {
  ...
  freeStackDesc:
    "Round-robin across the free models your connected providers serve. Zero cost, never stops.",
  paidPremiumTitle: "Paid Premium",
  paidPremiumDesc:
    "Round-robin across the paid models your connected providers serve. Top-tier models, distributed load.",
};
...
function ComboFormModal({ isOpen, combo, onClose, onSave, activeProviders, comboConfigMode }) {
  ...
  // The stack templates used to hardcode OmniRoute's own provider prefixes
  // (`agy/`, `kr/`, `if/`, `cu/`). Against this gateway every one of those steps
  // bound to nothing: the wizard unlocked, the combo saved, and then each step
  // reported "no live deployment" — a template that produced a broken combo.
  //
  // They are now built from the caller's REAL providers, taken from
  // /api/combos/builder/options, where each model carries `isFree` from the
  // catalog. So "Free Stack" means the caller's own free models, and the
  // template can only ever produce steps that resolve.
  const buildStackSteps = useCallback(
    (wantFree) => {
      const steps = [];
      for (const provider of builderProviders) {
        const matching = (provider.models || []).filter(
          (model) => Boolean(model.isFree) === wantFree && model.mode !== "embedding"
        );
        // Two per provider: enough to survive one model going down without
        // turning a one-click template into a 100-step combo.
        for (const model of matching.slice(0, 2)) {
          steps.push(
            buildPrecisionComboModelStep({
  ...
  const applyTemplate = (template) => {
    ...
    if (template.id !== "free-stack" && template.id !== "paid-premium") return;

    const steps = buildStackSteps(template.id === "free-stack");
    if (steps.length === 0) {
      // Say why nothing happened. Silently applying an empty step list would
      // look identical to the template not working at all.
      notify.error(
        template.id === "free-stack"
          ? getI18nOrFallback(
              t,
              "templateNoFreeModels",
              "None of your connected providers currently serve a free model. Add a provider key first, or pick models by hand."
            )
          : getI18nOrFallback(
              t,
              "templateNoPaidModels",
              "None of your connected providers currently serve a paid model. Add a provider key first, or pick models by hand."
            )
      );
      return;
    }
