...
export default function HealthPage() {
  ...
  // `system` is defaulted rather than destructured bare: this page is fed by a
  // bridge route, and any payload without a `system` key used to take the whole
  // screen down on `system.uptime` before the first paint. A health page that
  // crashes when health data is partial is the one page that must not.
  const {
    system = {},
    providerHealth,
    providerSummary,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          {/* The gateway is Python, so there is no Node version to report; the
              bridge sends null rather than a fabricated one. Hide the line
              instead of rendering "Node vnull". */}
          {system.nodeVersion ? (
            <p className="text-xs text-text-muted mt-1">
