...
export default function APIPageClient({ machineId }: Readonly<APIPageClientProps>) {
  ...
  // Seeded from the build-time public gateway URL rather than OmniRoute's old
  // default port. /api/network/info overwrites this on mount; the seed only has
  // to be right for the frame before that lands — and "http://localhost:20128"
  // was not, so the page briefly advertised a port nothing listens on.
  const [localApiUrl, setLocalApiUrl] = useState(
    () =>
      `${(process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:8000").replace(/\/+$/, "")}/v1`
  );
  const [lanUrls, setLanUrls] = useState<string[]>([]);
  // The gateway's own account of what it serves (backend /v1/capabilities,
  // relayed inside /api/network/info). Authoritative for which endpoint cards
  // are real; see supportedPaths below.
  const [capabilities, setCapabilities] = useState<{
    openai_compatible?: Array<{ path?: string; supported?: boolean }>;
    anthropic_compatible?: Array<{ path?: string; supported?: boolean }>;
  } | null>(null);
  ...
  // Which paths this gateway actually serves, straight from /v1/capabilities.
  // Null until the fetch lands; treated as "assume supported" for that frame so
  // the grid does not flash every card as unavailable on first paint.
  const supportedPaths = useMemo(() => {
    if (!capabilities) return null;
    const paths = new Set<string>();
    for (const group of [capabilities.openai_compatible, capabilities.anthropic_compatible]) {
      for (const entry of group ?? []) {
        if (entry?.supported && typeof entry.path === "string") paths.add(entry.path);
      }
    }
    return paths;
  }, [capabilities]);
  ...
  const isSupported = useCallback(
    (path: string) => !supportedPaths || supportedPaths.has(path),
    [supportedPaths]
  );

  // The endpoint count is now just "how many paths the gateway reports", rather
  // than a heuristic over the model list. The old version counted `responses`,
  // `completions`, `batch`, `files` and `image edits` as permanently available —
  // none of which this gateway implements — so the headline number was inflated
  // by five no matter what was configured.
  const availableEndpointCount = useMemo(
    () => (supportedPaths ? supportedPaths.size : 0),
    [supportedPaths]
  );
        ...
        <div className="mb-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
            <EndpointCard
              ...
              supported={isSupported("/v1/chat/completions")}
            ...
            <EndpointCard
              ...
              path="/v1/responses"
              supported={isSupported("/v1/responses")}
              models={endpointData.chat}
              copy={copy}
              copied={copied}
              baseUrl={currentEndpoint}
            ...
            <EndpointCard
              ...
              path="/v1/completions"
              supported={isSupported("/v1/completions")}
              models={endpointData.chat}
              copy={copy}
              copied={copied}
              baseUrl={currentEndpoint}
            ...
            <EndpointCard
              ...
              path="/v1/messages"
              supported={isSupported("/v1/messages")}
              models={null}
              badge="Anthropic"
              copy={copy}
              copied={copied}
        ...
        <div className="mb-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
            <EndpointCard
              ...
              title={t("embeddings")}
              path="/v1/embeddings"
              supported={isSupported("/v1/embeddings")}
            ...
            <EndpointCard
              ...
              supported={isSupported("/v1/images/generations")}
            ...
            <EndpointCard
              ...
              supported={isSupported("/v1/images/edits")}
              models={endpointData.images}
              copy={copy}
              copied={copied}
              baseUrl={currentEndpoint}
            ...
            <EndpointCard
              ...
              supported={isSupported("/v1/audio/transcriptions")}
            ...
            <EndpointCard
              ...
              supported={isSupported("/v1/audio/speech")}
            ...
            <EndpointCard
              ...
              supported={isSupported("/v1/music/generations")}
            ...
            <EndpointCard
              ...
              supported={isSupported("/v1/videos/generations")}
          ...
          <div className="mb-5">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
              <EndpointCard
                ...
                path="/v1/search"
                supported={isSupported("/v1/search")}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
            <EndpointCard
              ...
              title={t("rerank")}
              path="/v1/rerank"
              supported={isSupported("/v1/rerank")}
            ...
            <EndpointCard
              ...
              title={t("moderations")}
              path="/v1/moderations"
              supported={isSupported("/v1/moderations")}
            ...
            <EndpointCard
              ...
              path="/v1/batches"
              supported={isSupported("/v1/batches")}
              models={null}
              badge="OpenAI"
              copy={copy}
              copied={copied}
            ...
            <EndpointCard
              ...
              path="/v1/files"
              supported={isSupported("/v1/files")}
              models={null}
              copy={copy}
              copied={copied}
              baseUrl={currentEndpoint}
            ...
            <EndpointCard
              ...
              path="/v1/models"
              supported={isSupported("/v1/models")}
              models={null}
              copy={copy}
              copied={copied}
              baseUrl={currentEndpoint}
...
function EndpointCard({
  ...
  /** False when the gateway's /v1/capabilities does not list this path. */
  supported?: boolean;
}>) {
  const t = useTranslations("endpoint");
  ...
  // An endpoint this gateway does not serve must not look like one it does.
  // The card set is a static list inherited from OmniRoute, which serves a
  // superset; rendering all of them identically advertised paths that answer
  // 404, and the copy button handed the user a URL that cannot work.
  return (
    <div
      className={`border border-border rounded-lg p-3 transition-colors flex flex-col gap-2 ${
        supported ? "hover:bg-surface/30" : "opacity-50"
      }`}
      title={supported ? undefined : `${path} is not implemented by this gateway.`}
    >
