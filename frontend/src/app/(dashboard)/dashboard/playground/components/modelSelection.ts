...
export function pickDefaultModel(
  ...
  /**
   * Models the gateway's last probe found reachable. When supplied, the default
   * is drawn from here first: the plain alphabetical head of a provider's
   * catalog is regularly a model the upstream has retired (the NVIDIA list
   * opens with one that answers 410 Gone), so the Playground's out-of-the-box
   * state was a request guaranteed to fail. Falls back to the full list when
   * nothing is known to be callable, since a probably-dead model still beats no
   * selection at all.
   */
  ...
  if (callableModels && callableModels.length > 0) return callableModels[0];
