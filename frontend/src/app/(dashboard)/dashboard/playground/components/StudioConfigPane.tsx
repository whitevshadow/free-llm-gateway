...
export default function StudioConfigPane({ configState, setConfigState }: StudioConfigPaneProps) {
  ...
  // Paths the gateway reports it serves. Null until loaded — treated as "allow"
  // for that frame so the current selection is never briefly marked invalid.
  const [supportedPaths, setSupportedPaths] = useState<Set<string> | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        for (const group of [data?.openai_compatible, data?.anthropic_compatible]) {
          for (const entry of group ?? []) {
            if (entry?.supported && typeof entry.path === "string") paths.add(entry.path);
          }
        }
        setSupportedPaths(paths);
      } catch {
        // Leave null: without an answer, do not disable anything.
      }
    })();
    return () => controller.abort();
  }, []);

  const isEndpointSupported = (path: string) => !supportedPaths || supportedPaths.has(path);
  const {
    provider,
    setProvider,
    providerOptions,
  ...
  } = useGatewayModels(modelFilterKey);
  ...
  useEffect(() => {
    const next = pickDefaultModel(configState.model, availableModels, callableModels);
    if (next !== null) setConfigState({ ...configState, model: next });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [availableModels, callableModels, configState.model]);
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            {ENDPOINT_OPTIONS.map((opt) => {
              // ENDPOINT_OPTIONS is OmniRoute's full surface; this gateway serves
              // a subset. Offering the rest as ordinary choices meant picking one
              // produced a 404 with no hint that the endpoint was never available
              // — disable them and say so instead.
              const path = endpointToPath(opt.value);
              const enabled = isEndpointSupported(path);
              return (
                <option key={opt.value} value={opt.value} disabled={!enabled}>
                  {tp(`endpointOptions.${opt.labelKey}`)} — {path}
