...
export default function ChatTab({ configState, onMetricsUpdate }: ChatTabProps) {
  ...
  const doSend = async (chatMessages: Message[], appendIndex?: number) => {
    ...
    try {
      ...
      // Set when the stream carries an error frame instead of content.
          ...
          for (const line of lines) {
            ...
            if (line.startsWith("data: ")) {
              try {
                ...
                // A streaming request that fails upstream still returns HTTP 200:
                // the gateway reports the failure as `data: {"error": {...}}`
                // inside the stream, because the status line was already sent.
                // Reading only `delta.content` silently dropped every one of
                // those — a dead model or an exhausted key rendered as an empty
                // assistant bubble with a green 200 badge, which is the single
                // most misleading thing this screen can show.
                if (parsed.error) {
                  streamError = parsed.error.message || t("networkError");
                  continue;
                }
      ...
      // An in-stream error with no content is a failed request, so it is
      // reported exactly like the !res.ok path above: surface the message and
      // drop the empty assistant bubble. If some content did arrive before the
      // error, it is kept — a truncated answer plus the reason beats losing both.
      if (streamError && !assistantResponse) {
