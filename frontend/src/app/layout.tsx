...
export async function generateMetadata() {
  ...
  const instanceName = settings?.instanceName || "Free LLM Gateway";
  const customFaviconUrl = settings?.customFaviconUrl || settings?.customFaviconBase64;

  return {
    title: `${instanceName} — OpenAI-compatible multi-provider gateway`,
    description:
      "An OpenAI-compatible API gateway that aggregates multiple free LLM providers, and multiple keys per provider, behind a single endpoint.",
