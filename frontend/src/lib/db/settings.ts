...
/**
 * Read the `settings` rows from the local store, or none if there isn't one.
 *
 * Logged once rather than per call: with no local database this is the steady
 * state, not an incident, and a line per render would bury real errors.
 */
let warnedNoLocalStore = false;

function readLocalNamespaceRows(namespace: string): Array<{ key: string; value: unknown }> {
  try {
    const db = getDbInstance();
    return db
      .prepare("SELECT key, value FROM key_value WHERE namespace = ?")
      .all(namespace) as Array<{ key: string; value: unknown }>;
  } catch (error) {
    if (!warnedNoLocalStore) {
      warnedNoLocalStore = true;
      console.info(
        "[settings] No local settings store; using built-in defaults. " +
          "Gateway state is served by the FastAPI backend via src/bridge/. " +
...
function readLocalSettingsRows(): Array<{ key: string; value: unknown }> {
  return readLocalNamespaceRows("settings");
}

export async function getSettings() {
  // The local SQLite store is OmniRoute's own; this deployment's state lives in
  // the FastAPI gateway (see src/bridge/), so there is usually no database here
  // to open. Every value below already has a default, and the DB rows are only
  // an overlay on them — so an unavailable store degrades to "all defaults"
  // rather than taking down the caller.
  //
  // This is load-bearing well beyond settings: getSettings() is read by the root
  // layout's generateMetadata(), so letting it throw turns EVERY page, including
  // /login, into a 500 and locks the dashboard out entirely.
  const rows = readLocalSettingsRows();
...
export async function getProxyConfig() {
  // Same reasoning as getSettings(): the row set is an overlay on a complete
  // default, so no local store means the defaults stand.
  const rows = readLocalNamespaceRows("proxyConfig");
